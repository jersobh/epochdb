...
* **L2: Historical Archive (The Cold Tier)**
  Because RAM is finite, EpochDB implements "Epoch Checkpoints." Periodically, older memories are evicted from RAM and serialized to disk using `PyArrow`. They are stored in immutable `.parquet` files partitioned by time epochs. This ensures the engine remains lightweight while retaining access to unlimited historical data.
...
* **L2: Arquivo Histórico (Camada Fria / Cold Tier)**
  Como a RAM é finita, o EpochDB implementa "Epoch Checkpoints" (Pontos de Verificação de Épocas). Periodicamente, as memórias mais antigas são removidas da RAM e serializadas no disco rígido utilizando `PyArrow`. Elas são armazenadas em arquivos `.parquet` imutáveis, particionados por época (blocos de tempo). Isso garante que o motor continue leve, mantendo acesso ilimitado ao histórico.
