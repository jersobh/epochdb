...
* **L2: Historical Archive (The Cold Tier)**
  Because RAM is finite, EpochDB implements "Epoch Checkpoints." Periodically, older memories are evicted from RAM and serialised to disk using `PyArrow`. They are quantized via *int8 scalar quantization* (dynamically reducing storage requirements by 4x) and written into immutable `.parquet` files partitioned by time epochs. This ensures the engine remains lightweight while retaining access to unlimited historical data.
...
* **L2: Arquivo Histórico (Camada Fria / Cold Tier)**
  Como a RAM é finita, o EpochDB implementa "Epoch Checkpoints" (Pontos de Verificação de Épocas). Periodicamente, as memórias mais antigas são removidas da RAM e serializadas no disco rígido utilizando `PyArrow`. Elas são comprimidas via *quantização escalar int8* (reduzindo os requisitos de armazenamento em 4x) e gravadas em arquivos `.parquet` imutáveis, particionados por época (blocos de tempo). Isso garante que o motor continue leve, mantendo acesso ilimitado ao histórico.
